%% PreProcess script for Donnelly, 2018 
...
lmb_data = readtable('/home/pdonnelly/git/growthcurves_public/data/lmb_data.xlsx');
sub_map = mapparse('/home/pdonnelly/git/growthcurves_public/data/lmb_config.xlsx'); %mapparse initialized container map object
...
cntrl_sess_names = [11 12 13 14]; % based on study name variable coding in redcap
...
int_data{location,4} = 0; int_data{location,9} = 0; int_data{location,10} = 0;
...
% get rid of first intake session for record id #9
temp = find(ismember(int_data.record_id, 9));
int_data(temp(1), :) = [];
% get rid of first intake session for record id #35
temp = find(ismember(int_data.record_id, 35));
int_data(temp(1), :) = [];
% get rid of first intake session for record id #84
temp = find(ismember(int_data.record_id, 84));
int_data(temp(1), :) = [];
...
int_data.int_time_cen = center(int_data, 'int_time', 'record_id');
...
for test = 1:length(tests)
    ...
    cntrl_stats(test).lme = fitlme(cntrl_data, 'score ~ 1 + cntrl_days_cen + (1|record_id) + (cntrl_days_cen - 1|record_id)');
    % quadratic model fit
